            ...
            <div v-if="activeTab === 'products'" class="space-y-8">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="viewMode = 'grid'" :class="['p-2', viewMode === 'grid' ? 'text-primary' : 'text-gray-400']">
                            <Squares2X2Icon class="h-5 w-5" />
                        </button>
                        <button @click="viewMode = 'list'" :class="['p-2', viewMode === 'list' ? 'text-primary' : 'text-gray-400']">
                ...
                <div :class="['grid gap-6', viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1']">
                    ...
                    >
                        <div :class="['relative overflow-hidden rounded-lg', viewMode === 'list' ? 'h-32 w-32 flex-shrink-0' : 'mb-4']">
                            <img
                                :src="product.image"
                                :alt="product.name"
                        ...
                        <div :class="[viewMode === 'list' && 'flex-1']">
                            ...
                            <div class="mt-4 flex items-center justify-between">
                                <button @click="addToCart(product)" class="hover:bg-primary-dark rounded-lg bg-primary px-4 py-2 text-white">
                                    Ajouter au panier
                                </button>
                                <div class="flex gap-2">
                                    <button @click="addToWishlist(product.id)" class="rounded-lg border p-2 text-gray-600 hover:text-red-500">
                                        <HeartIcon class="h-5 w-5" />
                                    </button>
                                    <button @click="navigateToProduct(product.id)" class="hover:text-primary-dark text-primary">Voir détails</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="activeTab === 'reviews'" class="space-y-8">
                <div class="rounded-lg bg-white p-6 shadow-sm">
                    <div class="mb-6 flex items-center justify-between">
                        ...
                        <button @click="showReviewForm = true" class="hover:bg-primary-dark rounded-lg bg-primary px-4 py-2 text-white">
                            Écrire un avis
                        </button>
                    </div>
                    ...
                    <div v-if="showReviewForm" class="mb-6 rounded-lg border p-4">
                        ...
                        <form @submit.prevent="submitReview" class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea v-model="newReview.comment" rows="4" class="w-full rounded-lg border p-2"></textarea>
                            </div>
                            <div class="flex justify-end gap-4">
                                <button type="button" @click="showReviewForm = false" class="rounded-lg border px-4 py-2 hover:bg-gray-50">
                                    Annuler
                                </button>
                                <button type="submit" class="hover:bg-primary-dark rounded-lg bg-primary px-4 py-2 text-white">Publier</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div v-for="review in reviews" :key="review.id" class="border-b pb-6 last:border-0">
                            <div class="mb-4 flex items-center justify-between">
                                <div class="flex items-center">
                                    <img :src="review.user.avatar" :alt="review.user.name" class="mr-4 h-10 w-10 rounded-full object-cover" />
                                    <div>
