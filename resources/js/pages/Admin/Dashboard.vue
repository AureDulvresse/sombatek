    ...
    <AppLayout>
        <div class="container mx-auto py-8">
            ...
            <template v-else-if="user.role === 'shop'">
                <h1 class="text-3xl font-bold mb-8">Tableau de bord boutique</h1>
                <!-- Statistiques boutique, commandes, CA, etc. -->
                ...
                <!-- Graphique des ventes, etc. -->
            </template>
            <template v-else-if="user.role === 'promoter'">
                <h1 class="text-3xl font-bold mb-8">Tableau de bord démarcheur</h1>
                <!-- Statistiques sur les ventes générées, commissions, boutiques partenaires, etc. -->
                <div class="rounded-lg bg-white p-6 shadow text-center mb-8">
                    <div class="text-2xl font-bold">{{ stats.revenue.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' }) }}</div>
                    <div class="text-gray-500">Commissions générées</div>
                </div>
                <!-- Liste des boutiques partenaires, etc. -->
            </template>
            <template v-else>
                <h1 class="text-3xl font-bold mb-8">Tableau de bord client</h1>
                <!-- Historique de commandes, wishlist, etc. -->
                <div class="rounded-lg bg-white p-6 shadow text-center mb-8">
                    <div class="text-2xl font-bold">{{ stats.orders }}</div>
                    <div class="text-gray-500">Commandes passées</div>
                </div>
                <!-- Autres infos client -->
            </template>
        </div>
